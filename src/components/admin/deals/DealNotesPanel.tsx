...
import type { DealNote } from "@/lib/models/deal";
import { addDealNoteAction } from "@/lib/actions/deals.actions";
import { useToast } from "@/components/admin/ToastProvider";

export function DealNotesPanel({ dealId, notes }: { dealId: string; notes: DealNote[] }) {
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      await addDealNoteAction(dealId, trimmed);
      setValue("");
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5 sm:p-6">
      ...
      <p className="mt-1 text-xs text-muted">Private to CRM staff — never visible to the customer.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-2.5 sm:flex-row">
        <textarea
          ...
          rows={2}
          placeholder='e.g. "Customer requested revised payment schedule."'
