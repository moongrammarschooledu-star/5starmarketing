...
import type { ExpenseSearchFilters } from "@/lib/models/accounting";
import { expenseStatuses } from "@/lib/models/accounting";

export function ExpenseFilters({ filters }: { filters: ExpenseSearchFilters }) {
    ...
    <div className="rounded-2xl border border-border bg-surface p-4 sm:p-5">
      <form onSubmit={submitSearch} className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by number, description, vendor..." className="w-full rounded-lg border border-border bg-surface py-2.5 pl-9 pr-3 text-sm text-ink outline-none focus:border-primary" />
        </div>
