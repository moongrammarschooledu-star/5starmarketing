...
export function MaintenanceCommentThread({ requestId, comments }: { requestId: string; comments: MaintenanceComment[] }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        await addMaintenanceCommentAction(requestId, body.trim(), internalOnly);
        setBody("");
        router.refresh();
      } catch (e) {
  ...
  return (
    <div className="mt-3">
      <div className="space-y-2">
        {comments.map((c) => (
          <div key={c.id} className={`rounded-xl border p-3.5 ${c.internalOnly ? "border-amber-300 bg-amber-50" : "border-border bg-surface"}`}>
            ...
            {c.internalOnly && <p className="mt-1 text-[10px] font-bold uppercase text-amber-700">Internal only — never shown to customer</p>}
          </div>
        ))}
      ...
      <div className="mt-3 rounded-xl border border-border bg-surface p-3">
        ...
        <div className="mt-2 flex items-center justify-between">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-ink">
            <input type="checkbox" checked={internalOnly} onChange={(e) => setInternalOnly(e.target.checked)} className="h-4 w-4 rounded border-border text-primary focus:ring-primary" />
            Internal only
          </label>
