    ...
    <div className="mt-3 grid grid-cols-1 gap-4 overflow-x-auto pb-2 sm:grid-cols-2 lg:flex lg:gap-4">
      {leadStatuses.map((status) => {
        ...
        return (
          <div
            key={status}
            className={`rounded-2xl border p-3 lg:w-72 lg:shrink-0 ${COLUMN_TONE[status]}`}
          >
            ...
            <div className="mt-3 space-y-2.5">
              {items.length === 0 && (
                <p className="rounded-xl border border-dashed border-border bg-surface/60 p-4 text-center text-xs text-muted">
                  No leads here.
                </p>
              )}
              {items.map((lead) => (
