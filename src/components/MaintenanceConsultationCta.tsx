...
import { createMaintenanceInquiryLeadAction } from "@/lib/actions/maintenance.actions";

export function MaintenanceConsultationCta({ propertyId, propertyTitle }: { propertyId: string; propertyTitle: string }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      try {
        await createMaintenanceInquiryLeadAction({ name, phone, email: email || undefined, propertyId, message: `Property condition / maintenance consultation request for ${propertyTitle}.`, consent, company });
        setSuccess(true);
      } catch (e) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5 sm:p-6">
      <h3 className="flex items-center gap-2 font-heading text-base font-bold text-ink">
        <Wrench className="h-4.5 w-4.5 text-primary" /> Property Condition &amp; Maintenance
      </h3>
      <p className="mt-1 text-sm text-muted">Have questions about this property&apos;s condition or ongoing maintenance? Request a consultation.</p>

      {!open && !success && (
        <button type="button" onClick={() => setOpen(true)} className="mt-3 rounded-full bg-primary px-4 py-2 text-xs font-bold text-primary-foreground">
      ...
      {open && !success && (
        <div className="mt-4 space-y-3 rounded-xl bg-surface-muted p-4">
          ...
          <button type="button" onClick={submit} disabled={isPending} className="rounded-full bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground disabled:opacity-50">
