...
export function ConstructionDocumentManager({ projectId, documents, types }: { projectId: string; documents: DocumentRecord[]; types: DocumentType[] }) {
  ...
  function upload() {
    ...
    reader.onload = () => {
      ...
      startTransition(async () => {
        try {
          await uploadDocumentAction({ title: title.trim(), documentType, constructionProjectId: projectId, visibility, fileName: file.name, dataUri: reader.result as string });
  ...
  return (
    <div className="mt-3">
      <div className="rounded-2xl border border-border bg-surface p-5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          ...
          <select value={visibility} onChange={(e) => setVisibility(e.target.value as DocumentVisibility)} className="rounded-lg border border-border bg-surface px-3 py-2 text-xs text-ink outline-none focus:border-primary">
            ...
            <option value="ADMIN_CUSTOMER">Visible to Customer</option>
          </select>
        </div>
      ...
      <div className="mt-4 space-y-2">
          ...
          <div key={doc.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border bg-surface p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {doc.visibility === "ADMIN_CUSTOMER" && <span className="mt-0.5 inline-block rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-bold text-green-800">Visible to customer</span>}
              </div>
            </div>
            <div className="flex items-center gap-2">
        ...
        {documents.length === 0 && <p className="text-sm text-muted">No documents uploaded for this project yet.</p>}
      </div>
    </div>
  );
}
